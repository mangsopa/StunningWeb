        ...
        <div>
            <form method="POST" action="{{ route('admin.roles.update', $data->id) }}">
                ...
                <div class="py-4 px-4 text-black text-2lg">
                    ...
                    <!-- Permissions Role -->
                    <div class="mt-4">
                        <x-input-label for="permissions" :value="__('Permission Name')" />
                        <select class="js-example-basic-multiple" multiple="multiple"
                            style="z-index: 9999; position: relative;" name="permissions[]" id="permissions[]">
                            @foreach ($permissions as $permission)
                                <option @selected($data->hasPermissionTo($permission->name)) value="{{ $permission->name }}">
                   ...
                   <div class="mt-4">
                    ...
                    <textarea id="descriptions" name="descriptions" rows="3" cols="4" class="form-control"
                        placeholder="Description">{{ $data->descriptions}}</textarea>
